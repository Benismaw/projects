...
public class ClassType extends Type {
    ...
    public boolean sameType(Type otherType) {
        ...
        // 2 classes sont identiques si elles ont le même nom 
    ...
    public boolean isSubClassOf(ClassType potentialSuperClass) {
        ClassDefinition current = this.getDefinition();
        Symbol otherName = potentialSuperClass.getName();
        
        while (current != null) {
            if (current.getType().getName().equals(otherName)) {
                return true;
            }
            current = current.getSuperClass(); //on remonte vers la super-classe 
        }
        return false; 
        }
