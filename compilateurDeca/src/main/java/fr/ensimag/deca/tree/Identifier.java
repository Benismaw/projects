...
import fr.ensimag.ARM.Instructions.ARMLdrMem;
...
public class Identifier extends AbstractIdentifier {
    ...
    public Type verifyExpr(DecacCompiler compiler, EnvironmentExp localEnv,
        ...
        // Si pas trouvé localement et qu'on est dans une classe, on cherche dans l'nvironnement des membres
        if (expDef == null && currentClass != null) {
            expDef = currentClass.getMembers().get(name);
        }

        // Regle 0.1 identificateur non declaré
        if (expDef == null) {
            throw new ContextualError("Identificateur non déclaré : " + name + " (règle 0.1)", getLocation());
        }
    ...
    protected void codeGenExpr(DecacCompiler compiler, GPRegister register) {
        ...
        if (def.isField()) {
            ...
            int offset = this.getFieldDefinition().getIndex();
            compiler.addInstruction(new LOAD(new RegisterOffset(offset, register), register));
    ...
    protected void codeGenExprARM(DecacCompiler compiler, ARMGPRegister register) {
        ...
        compiler.getARMProgram().addInstruction(
            new ARMLdr(ARMRegister.R0, new ARMLabelOperand(new ARMLabel("=" + name)))
        );

        compiler.getARMProgram().addInstruction(
            new ARMLdrMem(register, ARMRegister.R0, 0)
        );
    }
    ...
    public Type verifyType(DecacCompiler compiler) throws ContextualError {
        ...
        // Regle 0.2 type inexistant
        if (typeDef == null) {
            throw new ContextualError("Type non déclaré : " + name + " (règle 0.2)", getLocation());
        }
